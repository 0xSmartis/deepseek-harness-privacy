...
Network-policy amendment: the [independent child-network decision](../architecture/2026-08-20-independent-child-network-policy.md) supersedes this note's statements that networking is outside `ctx.sandbox` and that `danger-full-access` bypasses the provider. This note remains authoritative for file modes, file escalation, runner attribution, and per-session file-mode state.
