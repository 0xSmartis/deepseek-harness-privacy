...
`packages/fs/fs-sandbox/`（`@deepseek-ai/dsh-fs-sandbox`）镜像 `bash-local`/`bash-sandbox` 的拆分：`SandboxedFileSystem extends LocalFileSystem`，注册为 `ctx.fs`，注入 `sandboxPolicy`。`resolve` 只映射身份，不作访问决策。元数据、内容、列出与变更操作都接受末尾可选的 `SandboxExecutionPolicy`；沙箱化提供方使用它在检查包含关系前立即重新规范化所访问的身份，并以该已检查身份进行委托。`lstat` 会规范化父目录并检查最终组件自身的位置，因此工作区内的符号链接可以暴露链接元数据而不会授权其外部目标，而指向工作区内部目标的外部符号链接仍会被拒绝：

- `read-only` 允许读取 `readableRoots(policy)` 下的目标，并直接拒绝 `writeText`/`editText`。
- `workspace-write` 允许读取 `readableRoots(policy)` 下的目标，并在 `writableRoots(policy)` 下实施变更。两个集合都包含 realpath 后的工作区根与平台临时区域（`/tmp`、`os.tmpdir()`）。规范拼写采用词法包含快速路径；当 Windows 以大小写不同的路径、长文件名或 8.3 短文件名表示同一目录时，系统会逐级遍历祖先目录并比较文件系统身份，而不会把限制弱化为依据文本前缀猜测包含关系。
- `danger-full-access` 不加围栏地委托。

拒绝是结构化的 `FS_SANDBOX_DENIED`，携带生效模式——区别于 `FS_PERMISSION_DENIED`（宿主 EACCES 是世界在拒绝；这里是策略在拒绝）。无文本推断：进程内围栏确切知道它拒绝了什么。该 seam 保持无会话依赖，裸的本地后端会忽略可选策略。`FileSystem.sandboxMode` 是能力事实（在基类与 `fs-local` 上为 `undefined`，在 `SandboxedFileSystem` 上为默认值），所以工具层按组合真相来宣告升权。
...
`dsh-tool-fs` 把当前会话解析成完整策略，并传给 `read`、`read_image`、`write` 与 `edit`，同时将 `FS_SANDBOX_DENIED` 映射为模型已从 bash 认识的标记：`[sandbox: file access denied under <mode> mode]`。当 `ctx.fs.sandboxMode` 在注册时报告一个受限模式，四个工具都宣告成对的 `sandbox_permissions` + `justification` 字段，并在执行前处理同样的 `ctx.approval` 请求。读取只宣告 `danger-full-access`，因为 `workspace-write` 与 `read-only` 具有相同的可读根目录；变更保留完整的加宽阶梯。系统会根据调用的生效模式检查是否严格加宽；授权只改变当前调用的模式，并保留其会话根目录。
...
- **`ctx.web` 的网络策略**——不属于本文件策略决策。进程沙箱现在会单独拒绝 Internet 协议套接字；按目标限定的 web 策略仍是独立工作。
- **`subagent-acp` 消费方**——沙箱 RFC 中未变的延后阶段。
- **单个会话中的额外读取或写入根目录**——解析后的策略携带一个主要 `SessionHeader.cwd`；ACP `additionalDirectories` 仍是独立的 bridge 与策略设计问题。
...
- **fs seam 上一个合并的 operation-options 对象**——因摩擦被否决：它会把既有取消参数移入另一个对象。末尾可选的 `SandboxExecutionPolicy` 匹配 bash 的携带并忽略模式，同时保持既有位置参数 API 不变。
...
- 在 `read-only` 下，元数据、文本、字节与列出操作只能在工作区和平台临时根目录下成功；变更操作会被拒绝。
- 在 `workspace-write` 下，读取与变更可在工作区和平台临时根目录下成功，其外均被拒绝。重新规范化会拒绝目标位于外部的工作区符号链接，并以已检查身份而非陈旧 target key 进行委托。
- 一个被拒的 fs 操作携带 `sandbox_permissions` + `justification` 重试一次时，会经组合的审批链提示；一次授权只让该次调用在更宽的模式下运行；rejected/cancelled/unavailable 各自产生逐字的 fail-closed 文案，且不执行文件系统操作。
- 一次 `permission` 预设切换同时管辖两个家族：会话切换模式后，下一次 bash 调用与下一次 fs 操作都从同一个 `sandbox/mode` 折叠遵循新模式。
- cwd 根目录不同的并发会话通过同一组服务实例携带不同策略；两个家族都不会缓存某个会话的根目录供下一次调用使用。
- 一次无 per-call 盖章、执行约束的直连 `ctx.fs` 操作会被围栏于部署默认值。
- `read`/`read_image`/`write`/`edit` 上的升权字段恰好在被挂载的 `ctx.fs` 受限时存在，在 `dsh-fs-local` 下不存在。
...
- **平台临时区域仍是宽泛根目录。** 在不破坏普通工具的前提下缩小临时访问范围之前，需要先提供每会话私有临时存储。
...
- 单元：`dsh-sandbox` 钉住可读与可写根目录派生及共享升权流程。`dsh-fs-sandbox` 在真实文件系统上覆盖每种读取原语、不跟随最终组件的两种符号链接方向、根目录外拒绝、新鲜身份委托、变更和按调用覆盖。`dsh-tool-fs` 钉住策略传播、schema 宣告、拒绝映射和审批结果；`dsh-tool-str-replace-editor` 通过组装后的提供方覆盖工作区外查看。
