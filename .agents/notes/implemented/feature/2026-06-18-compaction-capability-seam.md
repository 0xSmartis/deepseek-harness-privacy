...
- **Wiring**: shipped full agent presets load zero-config `dsh-token-meter`, `dsh-compaction-tool-result-pruner`, `dsh-compaction-basic`, then `dsh-command-compact`; service-wide defaults make manual compaction usable without repeated numeric policy, while each preset also states `auto: false` visibly.
