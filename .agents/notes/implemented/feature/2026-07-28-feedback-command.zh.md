...
位于 `packages/feedback/command-feedback/` 的 `@deepseek-ai/dsh-command-feedback` 通过 `ctx.commands` 注册一个全局 `feedback` 命令。`/feedback <text>` 在确认文本中包含接收会话 id 与当前会话共享披露；空输入或仅含空白的输入返回直接用法错误。处理器是同步的，只注入 `commands`，且没有任何配置。它不会创建或披露跨会话用户身份；[持久 id 移除决策](../simplification/2026-08-20-remove-persistent-anonymous-user-id.md)负责该否定保证。
...
随附的 `dsh` 基础组合无条件挂载该命令：没有配置，也不依赖 goal 栈。Web 客户端通过命令适配器暴露该命令。无头模式、ACP（Agent Client Protocol）和 JSON-RPC 不提供命令适配器，因此 `/feedback` 在那里不可用。已接受和被拒绝的反馈都不会创建 harness-home 身份。
