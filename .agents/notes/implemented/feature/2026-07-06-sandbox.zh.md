...
网络策略修订：[独立子进程网络决策](../architecture/2026-08-20-independent-child-network-policy.md)取代了本 Note 中「网络不属于 `ctx.sandbox`」以及「`danger-full-access` 绕过提供方」的陈述。本 Note 仍负责文件模式、文件升权、runner 归因与逐会话文件模式状态。
