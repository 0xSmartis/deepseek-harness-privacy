...
- **接线**：已交付的完整 agent preset 依次加载零配置的 `dsh-token-meter`、`dsh-compaction-tool-result-pruner`、`dsh-compaction-basic`，然后加载 `dsh-command-compact`；服务级默认值让手动压缩无需重复数值策略即可使用，同时每个 preset 也清晰写出 `auto: false`。
