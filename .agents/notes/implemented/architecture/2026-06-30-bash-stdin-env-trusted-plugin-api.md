...
2. **`env` merges after the minimal inherited base, so every explicit caller entry wins.** The later managed-namespace decision manages `DSH_*`: ambient entries are absent, and trusted `dshEnv` merges last, so an ordinary `env` entry can never displace a managed value. The complete order is `allowlist(process.env)` → `ENV_OVERRIDES` → ordinary `env` → `dshEnv`.
...
**Configurable ambient inheritance.** Rejected as speculative. Trusted callers can explicitly provide required values after the allowlisted base without weakening the default ambient protection.
