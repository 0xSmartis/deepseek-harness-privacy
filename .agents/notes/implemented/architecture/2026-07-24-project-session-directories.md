...
The encoded session id names an ownership directory rather than the transcript itself. `SessionPersistence.locate()` continues to return the fixed transcript path for trusted host consumers such as hook `transcript_path`; [model shells do not receive that path](../simplification/2026-08-20-keep-private-paths-out-of-model-shells.md). Discovery ignores other entries inside the session directory so the backend can add session-owned artifacts without another layout change.
