...
- Contained: a key left in `$DSH_HOME/.env` is materialized into `process.env` for trusted in-process consumers, but managed local children do not inherit it through the subprocess seam's [minimal operational environment](../simplification/2026-08-20-allowlist-inherited-child-environment.md). It remains a writable fallback below `.credentials.yaml`; a secret the Harness should own and isolate belongs in the managed document, which is never materialized.
