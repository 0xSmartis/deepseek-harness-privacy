...
编码后的会话 id 用于命名归属目录，而不是 transcript 文件本身。`SessionPersistence.locate()` 仍为钩子 `transcript_path` 等受信任 Host 消费方返回固定的 transcript 路径；[模型 shell 不会收到该路径](../simplification/2026-08-20-keep-private-paths-out-of-model-shells.md)。发现过程会忽略会话目录中的其他条目，因此后端以后添加会话自有产物时无需再次改变布局。
