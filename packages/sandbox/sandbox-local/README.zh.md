...
- **Windows 子进程网络只能实现部分强制执行**：ACL runner 没有独立 IP 网络机制。提供方报告 `networkEnforcement: 'partial'`；shell 与 terminal 消费方会拒绝 spawn，而不会静默放行网络。
