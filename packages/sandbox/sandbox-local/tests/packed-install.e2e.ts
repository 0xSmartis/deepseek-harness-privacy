...
describe.skipIf(!packable)('sandbox-local: packed-tarball distribution (publish-path rehearsal)', () => {
  beforeAll(async () => {
    ...
    writeFileSync(join(consumerDir, 'consumer.mjs'), `
      ...
      if (out.enforcing) {
        const confined = sandbox.confine(['bash', '-c', \`echo hi > \${workdir}/denied.txt\`], { mode: 'read-only', networkMode: 'deny-all', workspaceRoot: workdir })
        ...
        try {
          sandbox.confine(['true'], { mode: 'read-only', networkMode: 'deny-all', workspaceRoot: workdir })
