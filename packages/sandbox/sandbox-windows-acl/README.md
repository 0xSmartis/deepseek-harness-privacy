...
- **Writes are restricted; reads, network, and process visibility are not.** `WRITE_RESTRICTED` intersects write accesses only, so a confined child can read any caller-readable file and open sockets. The local provider reports `networkEnforcement: 'partial'`; shell and terminal consumers that require the default `deny-all` child-network policy reject the call before spawn on Windows. `read-only` mode cannot be expressed by this mechanism alone; pair it with a read-side policy or an AppContainer/`S-1-15-2` capability token for stronger file confinement.
