...
`readableRoots(policy)` and `writableRoots(policy)` derive the canonical workspace and temporary-directory allowlists used by in-process filesystem providers. `danger-full-access` bypasses both root checks.
