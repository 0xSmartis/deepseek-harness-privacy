...
- **Model-friendly terminal env** — `NO_COLOR=1 PAGER=cat GIT_PAGER=cat` (no `TERM=dumb`: that is a POSIX concept; `NO_COLOR` is honored by modern PowerShell renderers) merges after the service's minimal inherited environment; an explicit caller entry still wins, and managed `DSH_*` facts merge last.
