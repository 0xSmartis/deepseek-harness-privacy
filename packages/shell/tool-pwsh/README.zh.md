...
每次前台与后台模型 pwsh 调用都会通过共享的 [`dsh-shell-env`](../shell-env/) 注册表收到一份新收集的受信任 `DSH_*` 环境：`DSH_SHELL=1` 与 agent 的会话作用域 `DSH_SESSION_ID`。默认注册表不会公开 Harness 主目录或会话存储路径；受信任插件可以显式贡献其他已声明事实。系统会先删除继承的 `DSH_*` 值，再通过专用的 `ShellExecRequest.dshEnv` 通道传递快照；`process.env` 永不被修改。描述只教授通用的 `$env:DSH_*` 约定，而不是点名存储相关变量。
