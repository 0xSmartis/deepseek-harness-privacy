...
| 模式 | 文件影响 | 子进程网络 |
|---|---|---|
| `read-only`（默认） | 任何位置都不可写（在 `/dev` 中只有 `/dev/null` 节点可写，因此 `>/dev/null` 仍可正常工作） | 拒绝 IP 套接字；保留 Unix 域 IPC |
| `workspace-write` | 只能写入 `workspaceRoot` + `/tmp`（在 bwrap 下为临时目录，在 Landlock 下为宿主 `/tmp`，在 Seatbelt 下为 `/private/tmp` 加每用户临时目录） | 拒绝 IP 套接字；保留 Unix 域 IPC |
| `danger-full-access` | 文件写入不受限；提供方仍会包装命令 | 拒绝 IP 套接字；保留 Unix 域 IPC |
...
- **网络策略目前只有拒绝模式。** 该执行器尚不能批准按目标地址限定的访问。
- 进程机制（spawn、进程组终止、输出收集／spill、后台句柄、继承环境筛选）继承自 [`dsh-bash-local`](../bash-local/)；runner 选择位于 [`dsh-sandbox-local`](../../sandbox/sandbox-local/)。
...
- **`danger-full-access` 只作用于文件**：它仍要求可用的沙箱后端，以继承式拒绝子进程网络。
