...
| Mode | File effects | Child networking |
|---|---|---|
| `read-only` (default) | No writes anywhere (of `/dev`, only the `/dev/null` node is writable, so `>/dev/null` keeps working) | IP sockets denied; Unix-domain IPC available |
| `workspace-write` | Writes only under `workspaceRoot` + `/tmp` (ephemeral under bwrap, the host `/tmp` under Landlock, `/private/tmp` plus the per-user temp dir under Seatbelt) | IP sockets denied; Unix-domain IPC available |
| `danger-full-access` | File writes unrestricted; the provider still wraps the command | IP sockets denied; Unix-domain IPC available |

Semantics:

- **Denials are result facts.** A failed run whose stderr carries the selected backend's own file-denial dialect — the signatures the provider stamps on every wrap (EROFS text under bwrap, EACCES under Landlock, EPERM under Seatbelt) — is reported as `ShellRunResult.sandbox.denied: true` (conservative classification, read from the collected stderr tail). Every run carries its file mode, `networkMode`, and the provider's independent `enforcement` and `networkEnforcement` facts. This executor refuses to spawn unless network enforcement is `full`.
...
- Process mechanics (spawn, process-group kills, output collection/spill, background handles, inherited-environment filtering) are inherited from [`dsh-bash-local`](../bash-local/); runner selection lives in [`dsh-sandbox-local`](../../sandbox/sandbox-local/).
...
- **Network policy is deny-only** — destination-scoped access cannot yet be approved through this executor.
...
- **`danger-full-access` applies only to files** — it still requires a usable sandbox backend for inherited child-network denial.
