...
| `env` | `{}` | 显式指定的子进程环境，叠加在子进程 seam 的最小操作性继承环境之上。 |
...
生产环境会解析锁定的 `@openai/codex@0.147.0` 依赖所声明的 `codex` bin，并使用当前 Node 可执行文件启动该 JavaScript wrapper。Wrapper 会选择匹配的原生平台载荷；提供方既不检查也不回退 `PATH` 中的宿主 `codex`。项目本地的原生 Codex 配置继续通过父会话 cwd 保持权威，而用户配置、身份验证、端点以及所需的 `HOME` 或 `CODEX_HOME` 值必须无需环境状态即可发现，或通过提供方的 `env` 有意提供。提供方只覆盖选定线程的 approval／reviewer／sandbox 字段；其他项目、模型、provider、MCP、hook、skill 与账户设置仍由原生机制负责。本插件不选择模型、不创建 `CODEX_HOME`、不执行登录，也不探测账户。子进程 seam 只继承最小操作性环境，随后再应用显式 `env` 覆盖。
