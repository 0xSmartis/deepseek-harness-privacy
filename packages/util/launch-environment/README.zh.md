...
- **快照不是子进程授权**：每一层同样会被物化进 `process.env` 供进程内消费方使用，但 [`dsh-subprocess`](../../subprocess/subprocess/README.md) 只会把最小操作性允许列表转发给受管子进程，除非受信任消费方显式提供其他值。产品启动器的 [`.env` 约定](../../boot/app-boot/README.md#profiles) 会在物化之前拒绝 bootstrap 变量。
