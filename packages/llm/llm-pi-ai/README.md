      ...
      acme-gateway:
        ...
        credentialHeaders:
          X-Organization-Token:
            credentialEnv: ACME_ORGANIZATION_TOKEN
            scheme: Bearer
...
Supported profile fields are `apiKeyEnv`, `credentialHeaders`, `emptyHeaders`, `displayName`, `api`, `baseURL`, `models`, `modelOverrides`, `compat`, `defaultContextWindow`, `defaultMaxTokens`, `defaultInput`, `reasoning`, `thinkingBudgets`, `cacheRetention`, `transport`, `timeoutMs`, `websocketConnectTimeoutMs`, `streamIdleTimeoutMs`, `maxRequestImageBytes`, and `retryPolicy`. Header names and schemes use HTTP token syntax, names are unique case-insensitively, and Harness attribution names cannot be replaced. Each resolved profile retry policy is captured with that provider route; omission uses the shared bounded normal default of five retries. The stream-idle interval is a positive finite Node timer delay, defaults to five minutes, and covers only an outstanding provider read, not consumer think time. `maxRequestImageBytes` bounds one request's base64-encoded image payload (default 20MiB, a positive integer): every image in history is re-encoded into every request, so when the accumulated payload exceeds the bound, the oldest images are replaced by a fixed text placeholder until the request fits, keeping an image-heavy session serviceable instead of permanently rejected by a gateway request-size cap. The default leaves capacity for system prompts, history, tools, and JSON; deployments behind stricter gateways lower it per route.
...
Credentials never enter that collection. The harness resolves a route's API key and credential-header references through its own seam before the request reaches pi-ai, passing only the resolved request-local values to the SDK; `Models` therefore holds no credential store, and the harness keeps its fail-loud reference semantics. A route naming no API key resolves as configured-but-keyless and leaves the requirement to the protocol, which is where it actually lives.
...
Every request carries the shared attribution header from dsh-llm's `attributionHeaders()`, merged through pi-ai's `headers` stream option after credential resolution. Configuration cannot replace a Harness-owned attribution name. Provider-specific app-attribution headers are not synthesized. See [dsh-llm § App attribution](../llm/README.md#app-attribution-attributionts).
...
- **An unauthenticated route depends on its protocol** — naming no API-key credential resolves the route as configured-but-keyless, but pi-ai's OpenAI-compatible implementation still requires an API key or an `Authorization` header, so a local server that requires a placeholder uses `apiKeyEnv`; a custom authentication header uses `credentialHeaders` with a credential reference.
