...
请求关联仅限会话范围。携带 `GenerateOptions.sessionId` 的请求会通过 `x-deepseek-harness-session-id` 发送该确切值，缺少会话的直接调用则省略该标头。适配器不会生成或发送稳定的跨会话用户 id。会话标头会发送至解析后的 `baseURL`（包括已配置的 gateway），且不会进入请求正文或模型可见内容。
