...
Request correlation is session-scoped. A request carrying `GenerateOptions.sessionId` sends that exact value as `x-deepseek-harness-session-id`, while a direct call without a session omits the header. The adapter does not generate or send a stable cross-session user id. The session header goes to the resolved `baseURL`, including a configured gateway, and remains outside the request body and model-visible content.
