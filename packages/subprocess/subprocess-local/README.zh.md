...
- **显式环境是受信任授权**：消费方可以有意恢复继承基底省略的任意值，包括凭据、主目录／配置路径、代理和 `DSH_*`；部署组合必须只授予子进程所需的值。
