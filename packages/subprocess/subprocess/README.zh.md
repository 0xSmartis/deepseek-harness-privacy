...
- spawn 工作目录和可执行文件路径属于提供方的执行世界。`resolveExecutable(command, env?, signal?)` 验证绝对命令，或根据该执行世界继承的操作性 PATH 加显式覆盖来解析裸名称。
...
- `scrubbedParentEnv()` 是唯一一份共享的继承环境定义。它只保留命令查找（`PATH`、`PATHEXT`、`COMSPEC`）、操作系统启动（`SYSTEMROOT`、`WINDIR`）、区域设置（`LANG`、`LANGUAGE`、`LC_*`）、终端（`TERM`、`NO_COLOR`）、时区（`TZ`）和临时目录（`TEMP`、`TMP`、`TMPDIR`）变量；POSIX 上精确匹配，Windows 上不区分大小写。主目录／配置路径、代理、凭据、`DSH_*` 与任意部署变量都留在宿主侧，除非受信任消费方通过显式 `env` 提供；该显式环境在继承基底之后合并。本地普通 spawn 与终端 spawn 都应用该定义；拥有自身 spawn 的 SDK 管理传输可直接导入它。
