...
- Spawn working directories and executable paths belong to the provider's execution world. `resolveExecutable(command, env?, signal?)` verifies absolute commands or resolves bare names against that world's inherited operational PATH plus explicit overrides.
...
- `scrubbedParentEnv()` is the one shared inherited-environment definition. It retains only command lookup (`PATH`, `PATHEXT`, `COMSPEC`), OS bootstrap (`SYSTEMROOT`, `WINDIR`), locale (`LANG`, `LANGUAGE`, `LC_*`), terminal (`TERM`, `NO_COLOR`), timezone (`TZ`), and temporary-directory (`TEMP`, `TMP`, `TMPDIR`) variables; matching is exact on POSIX and case-insensitive on Windows. Home/config paths, proxies, credentials, `DSH_*`, and arbitrary deployment variables stay host-side unless the trusted consumer supplies them through explicit `env`, which merges after the inherited base. Local ordinary and terminal spawns both apply this definition; SDK-managed transports that own their spawn may import it directly.
